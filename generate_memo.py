...
def generate_pdf(row):
    ...
    # Generate unique filename using buyer and truncated title
    safe_name = "".join([c for c in row['buyer_name'] if c.isalnum() or c==' ']).strip()
    safe_title = "".join([c for c in row['title'] if c.isalnum() or c==' ']).strip()
    filename = f"{OUTPUT_DIR}/Memo_{safe_name.replace(' ', '_')}_{safe_title[:30].replace(' ', '_')}.pdf"
    pdf.output(filename)
    return filename
